...
def extract(movies_dict):
    Movies = {}
    count = 0
    for i in movies_dict:
        if movies_dict[count]["title"] is not None:
            Movies[count] = [movies_dict[count]["desc"], movies_dict[count]["showtimes"], movies_dict[count]["photos"],movies_dict[count]["date"]]
            count += 1
    return Movies
